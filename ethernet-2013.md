...
FPGAを使ったネットワークデバイス開発には，遅延，PPS，スループットなどを測る検証機が必要不可欠ですが，それらの値段はけっこう高かったりします．
今回作る回路は，ミニマム機能しかありませんが，そのような検証機の代わりの簡易デバッガーとして利用することができます．

また，最後には，回路の応用例として，10G/100G，そして，PCI Express回路設計について，本文で扱う1000BASE-Tと対比しながら簡単に触れたいと思います．
...
また，今回のサンプル回路は，10/M/100Mのスイッチングハブなどに挿しても動きません．


-------------------------------------
### 1000Base-T 回路設計の戦略を立てる

1000BASE-T (1Gbps)の回路を作るので，秒間1GBのデータを**遅れなく**入出力できる回路ができればいいといえます．

ここでは，125MHzのクロックで動く回路を考えてみます．
Ethernetポートの入り口から出口まで1Gbpsで内部転送が可能な125MHzの回路は，1クロックあたり8bitのデータを入力して，
何か処理し，8bitで出力出来れば1Gbpsがだせることになります．

    125 MHz * 8 bit = 1 Gbps

そんな回路をVerilog-HDLにしてみたのが以下のコードです．
...
data_inとdata_outは8bit幅のデータ転送用ポート，125M_clkは125MHzのクロック入力，sys_rstはリセット信号の入力です．

ここでは，8bitのdata_storeと呼ばれるレジスタを用意し，data_inの入力を125MHzの1クロックごとにdata_inの入力をdata_storeに代入しています．
さらに，data_storeレジスタをdata_outにwireしていて，data_storeの変更内容はそのままdata_outから読むことができるようにしています．
この回路は，125MHzで入力された8bitのデータを，8ns (125MHzなので1クロックは8ns) だけの遅延でデータを出力する回路ということになります．

ここでは，いきなり125MHzなんてクロックを持ってきてしまいましたが，実は，125MHzで8bitのデータ入力される仕様は，そのまま，今回扱うEthernet 1000BASE-TのPHYとMAC間のインタフェースであるGMIIそのものの仕様です．

まとめると，**125MHzで駆動する8bitのデータ入力を詰まることなく，常に一定の遅延によって8bitのデータ出力できる回路を設計できればEthernet 1000BASE-Tで動く回路**ということになります．

さらに余談ですが，これを156.25 MHzで動作してデータ入力を64 bitにすると，10GBASE-Rの仕様であるXGMIIになります．
別の言い方をすれば，156.25 MHz, 64bitで動作する回路は，秒間10GBのデータを処理し，64bitのデータを6.4 nsで駆動します．
つまり，10G以降の回路設計も基本は変わりません．
...
10Mや100Mは，いまさら感があるのと，コードも長くなるので扱いません．
